...
function contarCitas($conexion, $id_medico, $fecha = null, $turno = null, $busqueda = null){
    $sqlbusqueda = "";
    if($busqueda){
    ...
    if($fecha && $turno){
        ...
        mysqli_stmt_bind_param($preparacion_sql, "isss", $id_medico, $fecha, $turno, $turno);
    }elseif ($fecha){
        $sql = "SELECT COUNT(*) as total FROM citas c
                INNER JOIN pacientes p ON c.id_paciente = p.id_paciente
                INNER JOIN usuarios u ON p.id_paciente = u.id_usuario
        ...
        $preparacion_sql = mysqli_prepare($conexion, $sql);
        mysqli_stmt_bind_param($preparacion_sql, "iss", $id_medico, $turno, $turno);
    }else{
        $sql = "SELECT COUNT(*) as total FROM citas c
                INNER JOIN pacientes p ON c.id_paciente = p.id_paciente
                INNER JOIN usuarios u ON p.id_paciente = u.id_usuario
...
function obtenerCitas($conexion, $id_medico, $inicio, $registros, $fecha = null, $turno = null, $busqueda = null, $orden = null){
    $sqlbusqueda = "";
    if($busqueda){
        $sqlbusqueda = " AND (u.nombre LIKE '%$busqueda%' OR u.apellidos LIKE '%$busqueda%')";  
    }
    ...
    if($fecha && $turno){
        ...
        $preparacion_sql = mysqli_prepare($conexion, $sql);
        mysqli_stmt_bind_param($preparacion_sql, "isssii", $id_medico, $fecha, $turno, $turno, $registros, $inicio);
    }elseif ($fecha){
        ...
        $preparacion_sql = mysqli_prepare($conexion, $sql);
        mysqli_stmt_bind_param($preparacion_sql, "issii", $id_medico, $turno, $turno, $registros, $inicio);
    }else{
        $sql = "SELECT c.id_cita, c.motivo, c.fecha_cita, u.nombre, u.apellidos
                FROM citas c
                INNER JOIN pacientes p ON c.id_paciente = p.id_paciente
